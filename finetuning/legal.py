...
def _load_clean_legal_dataset():
    dataset = load_dataset("un_pc", "en-es", split="train")
    # Rows 0–40k are noisy (headers, table entries); quality is consistently good
    # from 40k onwards. Pre-slicing to 560k rows makes filtering fast while
    # still leaving a large clean pool well above the 50k we need for training.
    dataset = dataset.select(range(40000, 600000))
...
def preprocess_legal_function(examples, tokenizer, max_length: int = 512):
    ...
    for en_text, es_text in zip(examples["en"], examples["es"]):
        prompt = f"Translate the following English legal text to Spanish using formal legal terminology:\n\nEnglish: {en_text}\nSpanish: "
...
def legal():
    ...
    dataset = _load_clean_legal_dataset()
...
def evaluate_legal(checkpoint_path: str, num_samples: int = 200):
    ...
    # Recreate exact same filter+shuffle used in training to guarantee unseen samples
    dataset = _load_clean_legal_dataset()
    ...
    for example in test_data:
        ...
        prompt = (
            f"Translate the following English legal text to Spanish using formal legal terminology:\n\n"
            f"English: {en_text}\nSpanish: "
        )
    ...
    print(f"\n=== Legal Evaluation — {num_samples} unseen samples ===", flush=True)
